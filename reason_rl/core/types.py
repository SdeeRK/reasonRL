    ...
    def __getitem__(self, idx: int | slice):
        ...
        if isinstance(idx, slice):
            return PromptBatch(
                ...
                prompts_token_ids=self.prompts_token_ids[idx] if self.prompts_token_ids else None,
            )
...
class RolloutBatch:
    ...
    def summary(self) -> dict[str, float]:
        """Compute statistics of response lengths in the batch."""
        lengths = [len(item.response_ids) for group in self.groups for item in group.items]
...
class RewardBatch:
    ...
    Attributes:
        ...
        aux_metrics: Optional nested list of dictionary metrics, same shape as rewards.
                    Each dict contains named metrics (e.g., {"accuracy": 1.0, "format": 1.0}).
                    These are for logging/debugging and NOT used for advantage calculation.
    """

    groups: list[GroupRollout]
    rewards: list[list[float]]
    reward_details: list[list[dict[str, float]]] | None = None
    ...
    def summary(self) -> dict[str, float]:
        """
        对 reward_details 内的奖励条目按 key 取全局均值
        """
        ...
        for group_detail in self.reward_details:
            for item_detail in group_detail:
...
class TrainingBatch:
    ...
    def __getitem__(self, idx):
        ...
        return TrainingBatch(
            ...
            old_log_probs=self.old_log_probs[idx] if self.old_log_probs is not None else None,
    ...
    def to(self, device):
        ...
        return TrainingBatch(
            ...
            old_log_probs=self.old_log_probs.to(device) if self.old_log_probs is not None else None,
