...
class RolloutWorker:
    ...
    def __init__(self, model_config: ModelConfig, generation_config: GenerationConfig):
        ...
        self.engine = LLM(
            ...
            tensor_parallel_size=int(model_config.rollout_num_gpus)
            if model_config.rollout_num_gpus >= 1
            else 1,
            enforce_eager=True,
    ...
    def generate(
        ...
        sampling_params = SamplingParams(
            ...
            if generation_config.stop_tokens
            else ["</answer>"],
        ...
        for sample, request_output in zip(requests.samples, engine_outputs):
            ...
            groups.append(
                GroupRollout(sample=sample, prompt_ids=prompt_ids, items=items)
            )
