...
class DataProcessor:
    ...
    def to_prompt_batch(self, samples: list[Sample]) -> PromptBatch:
        ...
        """
        prompts_token_ids = [
            self.tokenizer(sample.prompt, add_special_tokens=False)["input_ids"]
    ...
    def to_training_batch(
        ...
        for sample in advantage_batch:
            input_ids = torch.tensor(
                sample.prompt_ids + sample.response_ids, dtype=torch.long
            )
            response_mask = torch.zeros_like(input_ids)
            response_mask[len(sample.prompt_ids) :] = 1
            attention_mask = torch.ones_like(input_ids)
            old_log_probs = torch.tensor(
                [0.0] * len(sample.prompt_ids) + sample.old_log_probs, dtype=torch.float
            )
